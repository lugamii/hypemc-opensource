...
public enum StringMaker {
    ...
    private StringMaker(char character, int length) {
    ...
    public static StringMaker getDefaultFontInfo(char c){
        for(StringMaker dFI : values()){
    ...
    public static String makeCenteredMotd(String message) {
		...
		for (int j = 0; j < i; j++) {
			...
			if (c == '§') {
				...
				StringMaker dFI = getDefaultFontInfo(c);
