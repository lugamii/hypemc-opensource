		...
		new BukkitRunnable() {
			public void run() {
				...
				for (final Player o : Bukkit.getOnlinePlayers()) {
					...
					int wins = bP.getData(DataType.HG_WINS).asInt();
					int kills = bP.getData(DataType.HG_KILLS).asInt();
...
//					sidebar.setText(1,  "§ehypemc.com.br");
	...
	public void createScoreboard(final Player p) {
		...
		sidebar.show();
		sidebar.setTitle(
				"§" + (colorList ? colors[new Random().nextInt(colors.length)] : new Random().nextInt(9)) + "§lHG");
		sidebar.setText(13, "");
		sidebar.setText(12, "§eEvento");
		sidebar.setText(11, " §fKills: §a" + 0);
		sidebar.setText(10, " §fWins: §2" + 0);
		sidebar.setText(9, "");
		sidebar.setText(8, "§eDouble Kit");
