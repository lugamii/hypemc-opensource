...
public class Lobby extends JavaPlugin {
	...
	private SimpleTopHologram topSwWins, topSwKills;
	...
	public void checkTopHolograms() {
		if (getConfig().get("hologram.swkills.location") != null) {
			double x = getConfig().getDouble("hologram.swkills.location.x");
			double y = getConfig().getDouble("hologram.swkills.location.y");
			double z = getConfig().getDouble("hologram.swkills.location.z");
			float yaw = getConfig().getInt("hologram.swkills.location.yaw");
			float pitch = getConfig().getInt("hologram.swkills.location.pitch");
			Location loc = new Location(getServer().getWorlds().get(0), x, y, z, yaw, pitch);

			topSwKills = new SimpleTopHologram("§B§lTOP 10 SKYWARS KILLS", "KILLS", loc, 12, DataCategory.SKYWARS,
					DataType.SKYWARS_SOLO_KILLS, 2400L);
			topSwKills.spawn();
		} else {
			topSwKills = new SimpleTopHologram("§B§lTOP 10 lSKYWARS KILLS", "KILLS", getServer().getWorlds().get(0).getSpawnLocation(), 12, DataCategory.SKYWARS,
					DataType.SKYWARS_SOLO_KILLS, 2400L);
			topSwKills.spawn();
		}

		if (getConfig().get("hologram.swwins.location") != null) {
			double x = getConfig().getDouble("hologram.swwins.location.x");
			double y = getConfig().getDouble("hologram.swwins.location.y");
			double z = getConfig().getDouble("hologram.swwins.location.z");
			float yaw = getConfig().getInt("hologram.swwins.location.yaw");
			float pitch = getConfig().getInt("hologram.swwins.location.pitch");
			Location loc = new Location(getServer().getWorlds().get(0), x, y, z, yaw, pitch);

			topSwWins = new SimpleTopHologram("§B§lTOP 10 SKYWARS WINS", "WINS", loc, 12, DataCategory.SKYWARS,
					DataType.SKYWARS_SOLO_WINS, 2400L);
			topSwWins.spawn();
		} else {
			topSwWins = new SimpleTopHologram("§B§lTOP 10 SKYWARS WINS", "WINS", getServer().getWorlds().get(0).getSpawnLocation(), 12, DataCategory.SKYWARS,
					DataType.SKYWARS_SOLO_WINS, 2400L);
			topSwWins.spawn();
		}
