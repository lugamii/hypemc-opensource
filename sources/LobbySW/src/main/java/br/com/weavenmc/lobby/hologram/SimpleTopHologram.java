...
public class SimpleTopHologram {
	...
	public void spawn() {
		...
		for (int i = 2; i < handle.length; i++) {
			handle[i] = handle[j].addLineBelow("§e" + j + ". §7Ninguém - §e0 " + suffix);
			++j;
		}
		spawned = true;
		...
		Bukkit.getScheduler().runTaskTimerAsynchronously(Lobby.getPlugin(), new RunnableHologramUpdater(), update, update);
	}
	...
	public void update() {
		...
		for (int i = 0; i < topPlayers.size(); i++) {
			...
			handle[j].setText(
					"§e" + (i + 1) + ". " + tag + nextTop.getName() + " §7- §e" + nextTop.getCash() + " " + suffix);
			++j;
		}
	}
